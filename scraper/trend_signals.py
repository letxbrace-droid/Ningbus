...
async def google_trends_scores(keywords: list[str], geo: str = "FR") -> dict[str, dict]:
    ...
    async def _fetch_one(kw: str) -> tuple[str, dict]:
        def _fetch():
            ...
            return kw, {"score": round(recent), "direction": direction}
    ...
    pairs = await asyncio.gather(*[_fetch_one(kw) for kw in keywords[:8]])
    return dict(pairs)
...
async def tiktok_organic_signal(keywords: list[str]) -> dict[str, dict]:
    ...
    async def _fetch_one(session: aiohttp.ClientSession, kw: str) -> tuple[str, dict]:
        async with sem:
            try:
                ...
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    ...
                    return kw, {"result_count": count, "level": level}
            except Exception as exc:
                logger.warning("TikTok DDG error for '%s': %s", kw, exc)
                return kw, {"result_count": None, "level": "unknown"}

    async with aiohttp.ClientSession(headers=_DDG_HEADERS) as session:
        pairs = await asyncio.gather(*[_fetch_one(session, kw) for kw in keywords[:10]])
    return dict(pairs)
        ...
        async with sem:
            try:
                ...
                async with session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as r:
                    ...
                    return kw, {"result_count": count, "competition_level": level}
            except Exception as exc:
                logger.warning("Amazon DDG error for '%s': %s", kw, exc)
                return kw, {"result_count": None, "competition_level": "unknown"}
