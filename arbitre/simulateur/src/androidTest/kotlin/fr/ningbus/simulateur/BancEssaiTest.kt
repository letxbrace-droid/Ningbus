...
class BancEssaiTest {
     ...
     * alors que rien n'est cassé. Le second essai, après un arrêt forcé et un
     * réglage remis à zéro, sert aux systèmes qui ne relient que sur
     * transition.
     */
    private fun activerLectureEcran() {
        if (serviceLie()) return

        shell("settings put secure enabled_accessibility_services $SERVICE")
        shell("settings put secure accessibility_enabled 1")
        if (attendreLiaison(15_000)) return
